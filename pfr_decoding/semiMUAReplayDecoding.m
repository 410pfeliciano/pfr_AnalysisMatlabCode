...
hold on
plot(linTime(semiMUAtimes2{1}), semiMUAtrains(semiMUAtimes2{1},1), '.K');
for kk = 2: size(semiMUAtrains,2)
    plot(linTime(semiMUAtimes2{kk}), semiMUAtrains(semiMUAtimes2{kk},kk)+kk-1, '.K');
end
hold off
xlabel('Time [sec]')
ylabel('Unit #')
axis([0 20 0 size(semiMUAtrains,2)+3])
subplot(413)
...
threshold = 3 * mean(median(MUAdensity ./ .6745), 2);
...
for jj = 1:size(below,1)
        ...
        for kk = 1: size(semiMUAtrains,2) % Spikes(0 or 1)
            MUAreact{1,jj}(:,7+kk) = semiMUAtrains(ind1:ind2,kk);
        end
end
...
for kk = 1: length(below)
    ...
    xlabel('Time [sec]')
    ylabel('MUA #')
    axis([-inf inf 0 size(semiMUAtrains,2)+3])
    yyaxis right
    ylabel('Position[m]')
    xlabel('Time [sec]')
    axis([-inf inf 0 1.3])
    ...
    subplot(414)
    plot(linTime(below(kk):abov(kk)), LFP2(below(kk):abov(kk)),'R')
    hold on
...
%% Visual Inspection
close all
for kk = 25%: size(MUAreactIdx,2)
    figure(kk)
    hold on
    s = plot3(MUAreact{1,kk}(:,1), MUAreact{1,kk}(:,2), ...
        ones(size(MUAreact{1,kk}(:,2),1),1),'-.r','LineWidth', 0.5);
    alpha(s,0.1)
%     plot3(TimeTrialbins{trial}(sepVect(1):sepVect(2)),y_fit,ones(size(y_fit,2),1),'-b','LineWidth', 2)
%     plot3(TimeTrialbins{trial}(sepVect(1):sepVect(2)),y_fit+2*delta,ones(size(y_fit,2),1),'--b','LineWidth', 1)
%     plot3(TimeTrialbins{trial}(sepVect(1):sepVect(2)),y_fit-2*delta,ones(size(y_fit,2),1),'--b','LineWidth', 1)
...
%%
% save('MUAposterior.mat', 'MUAreactLikelihoodLL','MUAreactLikelihoodRR', ...
%     'TimeTrialbins','tuningbins','MUAreact')
