...
%}
function [LFP,LFPt,fastGammaLFP] = fastGammaLFP(tetNum, ChNum, reduction, RedParm)
        flag = 'scale';  % Sampling Flag
        Fs = 30000/RedParm;  % Sampling Frequency
        N    = 500;      % Order
        win = blackman(N+1);
        RFc1 = 60;      % First Cutoff Frequency
        RFc2 = 100;      % Second Cutoff Frequency
if tetNum == 1
    if ChNum == 1
    ...
    elseif reduction == 1
        ...
        fastGammaHd = dfilt.dffir(Rb); % Ripple Filter
        fastGammaLFP = filtfilt(fastGammaHd.Numerator,1,LFP);
    end
    prompt = 'Do you want to convert the LFP from array to cell ? y/n: ';
    str = input(prompt,'s');
    if str == 'y'
        LFP = num2cell(LFP,1);
        fprintf('Converting LFP from array to cell\n')
    else
        fprintf('No array to cell conversion\n')
    end
