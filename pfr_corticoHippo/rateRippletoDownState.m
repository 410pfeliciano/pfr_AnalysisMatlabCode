...
Fs = 1000;
sint = 1/Fs;
ttbins = 0:sint:length(ctxData.lfps.raw)*sint-sint;
sleepRipplets= CA1pksRippleIdx*sint;
rippletrain = hist(sleepRipplets,ttbins);
preBelowDSidx = ctxData.dsIdx.ctxDownPreSlBelowIdx;
preAbovDSidx = ctxData.dsIdx.ctxDownPreSlAbovIdx;
...
%%
posBelowDownStateIdx = ctxData.dsIdx.ctxDownPostSlBelowIdx;
posAbovDownStateIdx = ctxData.dsIdx.ctxDownPostSlAbovIdx;
%%
matTs = 0.2;% time to analyzed
postt = -(matTs):sint:0;
...
plot(ttmat,posBelowRippRatesmooth, 'r', 'LineWidth',2)
ylabel('SWR rate ripple/sec')
xlabel('time rel. to Down-State(sec)')
ylim([0 3])
xlim([-inf 0])
