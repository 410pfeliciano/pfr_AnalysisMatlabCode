% LFP of Interest
CA1LFP = detrend(ch36Tet12CA1;); % raw LFP
...
hibfiltRipple = abs(hilbert(CA1LFPripple)); % amplitude envelope of ripple-band filtered LFP
sqfiltLFP= hibfiltRipple.^2; % calculating the power of the filtered LFP
smsqfiltLFP = smoothdata(sqfiltLFP,'gaussian',filtWin/sint); % smoothing the power data
zHippRippleEnv = zscore(smsqfiltLFP);
...
subplot(6,1,3)
plot(time(xt1:xt2),zHippRippleEnv(xt1:xt2),'b') 
axis tight
legend('Ripple Envp Z-score','Location', 'best')
legend boxoff
box off
% set(gca,'xtickLabel',[])
ylabel('z-score')
...
subplot(613)
plot(time(xt1:xt2), zHippRippleEnv(xt1:xt2),'Color', 'k')
hold on
for kk = 1: length(interBelowIdx)
    plot(time(below(interBelowIdx(kk)):abov(interBelowIdx(kk))),...
       zHippRippleEnv(below(interBelowIdx(kk)):abov(interBelowIdx(kk))),...
        'r', 'LineWidth',1)
end
axis([time1 time2 -inf inf])
...
subplot(616)
plot(time(xt1:xt2),CA1spkDensity(xt1:xt2),'k','LineWidth',1) 
hold on
for kk = 1: length(interBelowIdx)
    plot(time(below(interBelowIdx(kk)):abov(interBelowIdx(kk))),...
       CA1spkDensity(below(interBelowIdx(kk)):abov(interBelowIdx(kk))),...
        'r', 'LineWidth',1)
end
hold off
xlabel('Time[sec]')
ylabel('spks/s')
xlim([time1 time2])
%% Plotting position to define the running period
figure
subplot(311)
plot(positiondata.poscm(:,1),positiondata.poscm(:,2),'k.')
...
%LFPs
CA1Data.lfps.raw = CA1LFP;
CA1Data.lfps.ripple = CA1LFPripple;
CA1Data.lfps.rippleEnvZscore =  zHippRippleEnv; % ripple power envp z-score
CA1Data.lfps.rippleEnvSmooth = smsqfiltLFP; % ripple power envelop smooth
% MUA
CA1Data.spikes.Id = CA1TetIdx;
CA1Data.spikes.density = CA1spkDensity;
% Down-State Candidates Idx
CA1Data.rippleIdx.CA1RipSlAbovIdx = Sleepabov;
CA1Data.rippleIdx.CA1RipSlBelowIdx = Sleepbelow;
CA1Data.rippleIdx.CA1RipPreSlAbovIdx = PreSleepabov;
CA1Data.rippleIdx.CA1RipPreSlBelowIdx = PreSleepbelow;
CA1Data.rippleIdx.CA1RipPostSlAbovIdx = PostSleepabov;
CA1Data.rippleIdx.CA1RipPostSlBelowIdx = PostSleepbelow;
CA1Data.rippleIdx.CA1RipRUNbelow = RUNbelow1;
CA1Data.rippleIdx.CA1RipRUNRUNabov = RUNabov1;
