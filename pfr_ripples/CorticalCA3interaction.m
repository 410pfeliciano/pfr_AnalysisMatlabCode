...
hippRippleenv = smoothdata(abs(hilbert(CA3LFPripple)),'gaussian',smWin*Fs); 
...
%    figure
%    subplot(611)
%    plot(time(CA3RipSlBelowIdx(kk) - eventExt : CA3RipSlAbovIdx(kk)+ eventExt), ...
%        CTXLFP(CA3RipSlBelowIdx(kk) - eventExt : CA3RipSlAbovIdx(kk)+ eventExt),'k')
%    axis([-inf inf, -inf inf])
%    
%    subplot(612)
%    plot(time(CA3RipSlBelowIdx(kk) - eventExt : CA3RipSlAbovIdx(kk)+ eventExt),...
%        ctxMUAenvpsm(CA3RipSlBelowIdx(kk) - eventExt : CA3RipSlAbovIdx(kk)+ eventExt),'k')
%    axis([-inf inf, 0 inf])
%    
%    subplot(613)
%    plot(time(CA3RipSlBelowIdx(kk) - eventExt : CA3RipSlAbovIdx(kk)+ eventExt), ...
%        CA3LFP(CA3RipSlBelowIdx(kk) - eventExt : CA3RipSlAbovIdx(kk)+ eventExt),'r')
%    axis([-inf inf, -inf inf])
%    
%    subplot(614)
%    plot(time(CA3RipSlBelowIdx(kk) - eventExt : CA3RipSlAbovIdx(kk)+ eventExt), ...
%        CA3LFPripple(CA3RipSlBelowIdx(kk) - eventExt : CA3RipSlAbovIdx(kk)+ eventExt),'r')
%    axis([-inf inf, -inf inf])
%    
%    subplot(615)
%    plot(time(CA3RipSlBelowIdx(kk) - eventExt : CA3RipSlAbovIdx(kk)+ eventExt),...
%        ZhippRippleenv(CA3RipSlBelowIdx(kk) - eventExt : CA3RipSlAbovIdx(kk)+ eventExt),'r')
%    axis([-inf inf, -inf inf])
%    
%    subplot(616)
%     plot(time(CA3RipSlBelowIdx(kk) - eventExt : CA3RipSlAbovIdx(kk)+ eventExt),... 
%         CA3spkDensity(CA3RipSlBelowIdx(kk) - eventExt : CA3RipSlAbovIdx(kk)+ eventExt),... 
%         'r','LineWidth',2) 
%     xlabel('Time[sec]')
%     ylabel('spks/s')
%     axis tight
...
peakRipple = zeros(size(CA3RipSlBelowIdx,1),1);
for kk = 1: size(CA3RipSlBelowIdx,1)-1
   [R, peakRipple(kk)] = max(ZhippRippleenv(CA3RipSlBelowIdx(kk): CA3RipSlAbovIdx(kk)));
end
% this is to obtain the center indice based on the max ripple power peak
peakRippleIdx = zeros(size(peakRipple,1),1);
for kk = 1: size(CA3RipSlBelowIdx,1)-1
   v = CA3RipSlBelowIdx(kk): CA3RipSlAbovIdx(kk);
   peakRippleIdx(kk) = v(peakRipple(kk));
end
%% Before and after distributions
maxRipplepks = zeros(size(CA3RipSlBelowIdx,1),1);
maxMUApks = zeros(size(CA3RipSlBelowIdx,1),1);
maxMUActx = zeros(size(CA3RipSlBelowIdx,1),1);
for kk = 1: size(CA3RipSlBelowIdx,1)-1
...
xlabel('Max MUA ripple activation (Spk/sec)')
ylabel('Probability Density')
legend([h h1 h2],'Ripple Peak Z-Score','CA3 MUA','Ctx MUA')
legend('boxoff')  
box off
hold off
...
pksRippleIdx(maxRipplepks < 2 | maxMUApks < 20) = [];
ctxMUA = maxMUActx;
ctxMUA(maxRipplepks < 2 | maxMUApks < 20) = [];
% to use to calculate the ripple rate according to down-states
CA3pksRippleIdx = pksRippleIdx;
save('CA3pkssleeprippIdx.mat','CA3pksRippleIdx')
...
c = colorbar;
c.Label.String = 'CA3 Spike Rate (spk/sec)';
c.FontSize = 11;
% colorbar('southoutside')
axis xy
ylim([0 inf])
...
xlabel('Time relative to SWR peak (sec)')
ylabel('MUA')
legend([ctx,hipp],'Ctx MUA', 'CA3 MUA')
legend boxoff
axis([-inf inf 0 70])
set(gca,'FontSize',11)
...
ylabel('Probability Density')
legend([h, h1, h2], 'Ctx MUA Before CA3 Ripple',...
    'Ctx MUA During CA3 Ripple','Ctx MUA After CA3 Ripple')
legend('boxoff')  
set(gca,'FontSize',11)
box off
...
for kk = 650:680 %: size(,1)
   ...
   subplot(614)
   plot(time(pksRippleIdx(kk) - eventExt : pksRippleIdx(kk)+ eventExt), ...
       CA3LFPripple(pksRippleIdx(kk) - eventExt : pksRippleIdx(kk)+ eventExt),'r')
   axis([-inf inf, -inf inf])
