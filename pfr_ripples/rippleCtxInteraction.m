...
for kk = 1: length(interBelowIdx)
    plot(time(belowIdx(interBelowIdx(kk)):abovIdx(interBelowIdx(kk))),...
        CA1LFP(belowIdx(interBelowIdx(kk)):abovIdx(interBelowIdx(kk))),...
        'r', 'LineWidth',1)
end
axis([time1 time2 -inf inf])
...
% interval = Fs;
% overlap = round(Fs*0.95);
% nfft = length(CA1LFP(xt1:xt2));
[S, F, T,P] = spectrogram(CA1LFP(xt1:xt2),interval, overlap, nfft, Fs);
...
% CA1 activity
hippRippleenv = smoothdata(abs(hilbert(ch17Tet2LFPripple)),'gaussian',smWin*Fs);
ZhippRippleenv = zscore(hippRippleenv);
hippRipple = ch17Tet2LFPripple;
...
for kk = 1:20 %: size(,1)
   ...
   subplot(613)
   plot(time(belowIdx(kk) - eventExt : abovIdx(kk)+ eventExt), ...
       CA1LFP(belowIdx(kk) - eventExt : abovIdx(kk)+ eventExt),'r')
   axis([-inf inf, -inf inf])
   ...
   subplot(616)
    plot(time(belowIdx(kk) - eventExt : abovIdx(kk)+ eventExt),... 
        CA1spkDensity(belowIdx(kk) - eventExt : abovIdx(kk)+ eventExt),... 
        'r','LineWidth',2) 
    xlabel('Time[sec]')
    ylabel('spks/s')
    axis tight
...
pksRippleIdx(maxRipplepks < 4 | maxMUActx > 100) = [];
ctxMUA = maxMUActx;
ctxMUA(maxRipplepks < 4 | maxMUActx > 100) = [];
